...
export const EmployeePortalView: React.FC<EmployeePortalViewProps> = ({
  ...
  if (!employee) {
    return (
      <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center space-y-3">
        ...
        <h3 className="text-lg font-bold text-slate-900">Çalışan Profili Eşleşmedi</h3>
        <p className="text-xs text-slate-500">
          Mevcut kullanıcı hesabınız ({currentUser?.email}) aktif bir çalışan kaydıyla eşleşmiyor. Lütfen üstteki kullanıcı değiştiriciden bir çalışan hesabı seçin.
        </p>
      </div>
    );
  }
  ...
  const getRestaurantName = (restId: string) => {
    ...
    if (restId === 'rest-2') return 'Trattoria Milano';
    return restaurant?.name || 'Bistro Bella';
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-slate-300 mt-1">
                ...
                <span className="flex items-center gap-1 text-slate-300">
                  ...
                  {employee.isSharedStaff ? 'Bistro Bella & Trattoria Milano' : restaurant?.name}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span>Vardiya Listesini Yazdır</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6 pt-6 border-t border-slate-800">
          <div className="bg-slate-800/60 p-3.5 rounded-xl border border-slate-700/60">
            ...
            <div className="text-xl font-extrabold text-blue-400 mt-1">
              {totalHours} saat / maks {employee.maxWeeklyHours}s
            </div>
          </div>
          ...
          <div className="bg-slate-800/60 p-3.5 rounded-xl border border-slate-700/60">
            ...
            <div className="text-xl font-extrabold text-emerald-400 mt-1">
              ${estimatedEarnings} (${employee.hourlyRate}/saat)
            </div>
          </div>

          <div className="bg-slate-800/60 p-3.5 rounded-xl border border-slate-700/60">
            <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block">
              Atanan Vardiya Sayısı
            </span>
            <div className="text-xl font-extrabold text-white mt-1">
              {myShiftsThisWeek.length} Vardiya
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
            ...
            <span>Haftalık Vardiya Takvimi (2 Şube Dağılımı)</span>
          </h2>
          <span className="text-xs text-slate-500 font-medium">
            Müsaitliğinize göre atanmış vardiyalar
          </span>
        </div>
